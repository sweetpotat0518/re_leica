...
mobModal = function(){
	...
	var thumUrl = '../img/camera_view/';
	var bigUrl = '../img/view_big/';
  ...
  var cameraBox = $('.cameraBox');
	var cameraModal = $('.camera_modal');
	var cameraModalBtn = cameraModal.find('button');
	var cameraBoxUl = cameraBox.find('ul');

	// var cameraBoxLl = '<li><a href="#"><span></span></a></li>';

	// for(i=0; i<galleryList.thumList.length; i++){
	// 	cameraBoxUl.append(cameraBoxLl);
		
	// 	cameraBoxUl.find('li').eq(i).children('a').css({
	// 		backgroundImage:'url('+ thumUrl + galleryList.thumList[i] +')'
	// 	});
	// }

	var n = 0;
	var cameraBoxLlLink = cameraBoxUl.find('li').children('a');

	cameraBoxLlLink.on('click', function(e){
		e.preventDefault();
		var itIndex = $(this).parent().index();
		n = itIndex;

		cameraModal.find('.camera_big').css({backgroundImage:'url('+ bigUrl + galleryList.bigList[itIndex] +')', backgroundSize:'70%', backgroundPosition:'50% 50%', backgroundRepeat:'no-repeat',});
	
		// fadeIn 을 하고 방향키로 다음이미지 넘기기
		cameraModal.fadeIn(function(){
			$(window).on('keyup', function(evt){
				...
				if(keyCode == 37){
					...
					cameraModal.find('.camera_big').css({backgroundImage:'url('+ bigUrl + galleryList.bigList[n] +')'});
				}else if(keyCode == 39){
					n += 1;
					...
					// 빠져 나가고 포커스
					cameraModal.fadeOut(400, function(){
						cameraBoxLlLink.eq(0).focus();
					});
				}
	...
	cameraModalBtn.on('click', function(e){
		e.preventDefault();
		cameraModal.fadeOut(400);
	});
};
